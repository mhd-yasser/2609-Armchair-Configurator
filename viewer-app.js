...
const configs={
  ...
  desk:{title:'Axis Executive Desk',file:asset('desk/model/Desk.glb'),download:'VREEL_Axis_Executive_Desk.glb',groups:{
    ...
    frontPanel:group('Ön Panel',['Material__2147482989'],[...wood,...solids,...glass],'original',node=>node.name.startsWith('FrontPanel')),
...
function updateDimensions(){const box=document.querySelector('#dimensions-overlay');if(box.hidden)return;const dims=viewer.getDimensions();if(!dims)return;box.innerHTML=`<div class="measurement-readout">Genişlik: ${Math.round(dims.x)} cm · Yükseklik: ${Math.round(dims.y)} cm · Derinlik: ${Math.round(dims.z)} cm</div>`;}
